<?php
	class Conseil{
		function __construct(){
			?>
			</div> <!--fin wrapp-->
			<div id="conseilInstruction">
				<u>Conseil de Navigation:</u>
				<ul><table>
					<tr>
						<td width=300px><li>Pour poster une annonce: </li></td>
						<td>Cliquer sur l'onglet "<i>Créer une annonce</i>"</td>
					<tr>
					<tr>
						<td><li>Pour réaliser une transaction direct:</li></td>
						<td>Cliquer sur l'onglet "<i>Transaction Direct</i>"</td>
					</tr>
					<tr>
						<td><li>Pour visualiser une annonce:</li></td>
						<td>1) Pour une annonce spécifique: faites une recherche via la barre de recherche en haut</td>
					</tr>
					<tr>
						<td></td>
						<td>2) Ou cliquez sur une des annonces publiées sur la page d'Accueil</td>
					</tr>
					<tr>
						<td><li>Pour modifier mes paramètres:</li></td>
						<td>Cliquer sur l'onglet "<i>Mon Compte</i>"</td>
					</tr>
				</table></ul>

			</div>
			<?php 
		}	
	}
?>